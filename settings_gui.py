...
def search_school(api_key, name):
    resp = requests.get(
        ...
        params={"KEY": api_key, "Type": "json", "SCHUL_NM": name, "pSize": 20},
        timeout=10,
    )
...
class App(tk.Tk):
    def __init__(self):
        ...
        # 창 크기를 590x810으로 확장하여 글씨 크기 확대에 대응
        self.geometry("590x810")
        try:
        ...
        tab_school = ttk.Frame(notebook)
        tab_bg = ttk.Frame(notebook)
        tab_design = ttk.Frame(notebook)
        tab_about = ttk.Frame(notebook)

        notebook.add(tab_school, text=" 학교/학급 설정 ")
        notebook.add(tab_bg, text=" 배경 사진 설정 ")
        ...
        # ----------------------------------------------------
        about_text = (
            "School Wallpaper Sync\n\n"
            "Automatically syncs your desktop wallpaper with your school's daily timetable and meals.\n"
            "Designed with elegance and readability in mind.\n\n"
            "Version: 1.0.0\n"
            "Developer: do-hye\n"
            "License: MIT License\n\n"
            "Enjoy a more productive and beautiful desktop environment!"
        )
        ttk.Label(tab_about, text=about_text, font=("맑은 고딕", 11), justify="center").pack(expand=True, fill="both", padx=20, pady=40)

        # ----------------------------------------------------
        # Tab 1: 학교/학급 설정
        # ----------------------------------------------------
        ttk.Label(tab_school, text="NEIS Open API 키", font=("맑은 고딕", 11, "bold")).pack(anchor="w", **pad)
        self.api_key_var = tk.StringVar(value=self.cfg.get("api_key", ""))
        ttk.Entry(tab_school, textvariable=self.api_key_var, width=54).pack(anchor="w", padx=15)

        ttk.Label(tab_school, text="학교 이름", font=("맑은 고딕", 11, "bold")).pack(anchor="w", **pad)
        search_row = ttk.Frame(tab_school)
        ...
        self.result_list = tk.Listbox(tab_school, height=6, width=54, font=("맑은 고딕", 11))
        self.result_list.pack(anchor="w", padx=15, pady=6)

        gc_row = ttk.Frame(tab_school)
        gc_row.pack(anchor="w", padx=15, pady=8, fill="x")
        ...
        # Tab 2: 배경 사진 설정
        # ----------------------------------------------------
        ttk.Label(tab_bg, text="배경 사진 선택 모드", font=("맑은 고딕", 11, "bold")).pack(anchor="w", **pad)
        
        self.photo_mode = tk.StringVar(value=self.cfg.get("bg_photo_mode", "auto"))
        
        ttk.Radiobutton(tab_bg, text="자동으로 매일 순환 (감성 사진 8종)", value="auto",
                         variable=self.photo_mode, command=self.update_bg_ui_state).pack(anchor="w", padx=15, pady=4)
                         
        ttk.Radiobutton(tab_bg, text="감성 사진 8종 중 직접 고정 선택", value="preset",
                         variable=self.photo_mode, command=self.update_bg_ui_state).pack(anchor="w", padx=15, pady=4)
                         
        preset_row = ttk.Frame(tab_bg)
        ...
        ttk.Radiobutton(tab_bg, text="내 컴퓨터 사진 파일 사용", value="custom",
                         variable=self.photo_mode, command=self.update_bg_ui_state).pack(anchor="w", padx=15, pady=4)
                         
        photo_row = ttk.Frame(tab_bg)
    ...
    def on_search(self):
        ...
        if not api_key or not name:
            messagebox.showwarning("입력 필요", "API 키와 학교 이름을 입력해주세요.")
            return
        try:
            self.search_results = search_school(api_key, name)
        except Exception as e:
    ...
    def on_apply(self):
        ...
        if not api_key or not grade or not classnm:
            messagebox.showwarning("입력 필요", "API 키, 학년, 반을 모두 입력해주세요.")
            return
